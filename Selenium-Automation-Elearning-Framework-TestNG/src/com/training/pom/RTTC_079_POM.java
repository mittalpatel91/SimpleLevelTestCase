...
public class RTTC_079_POM {
	private WebDriver driver; 

	public RTTC_079_POM(WebDriver driver) {
		this.driver = driver; 
	...
	public void successCatMsg() {					//Verify Success Message for Category
		assertEquals(this.Msg.getText().replace("\n×", ""),"Success: You have modified categories!");
	}
	
	@FindBy(xpath="//div[@class='alert alert-danger']")//Locate warning message
	private WebElement warnMsg;

	public void warnMsg() {
		System.out.println("Error Message from the page: "+ this.warnMsg.getText().replace("\n×", ""));
		assertEquals(this.warnMsg.getText().replace("\n×", ""),"Warning: Please check the form carefully for errors!");
	}
...
//		this.modelopt.click();
	...
	public void successProdMsg() {					//Verify Success Message for Product
